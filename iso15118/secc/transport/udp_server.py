...
from iso15118.shared.network import (get_nic, SDP_SERVER_PORT,
                                     SDP_MULTICAST_GROUP)
...
class UDPServer(asyncio.DatagramProtocol):
    ...
    async def create(session_handler_queue: asyncio.Queue) -> 'UDPServer':
        ...
        multicast_group_bin = socket.inet_pton(socket.AF_INET6,
                                               SDP_MULTICAST_GROUP)
        ...
        try:
            ...
            logger.exception("Could not assign an interface for the UDP "
                             "server, unable to find network interface card. "
                             f"{exc}")

        interface_idx = socket.if_nametoindex(nic)
        join_multicast_group_req = (multicast_group_bin +  # address + interface
                                    struct.pack('@I', interface_idx))
        ...
        logger.debug("UDP server started at address "
    ...
    async def rcv_task(self, timeout: int = None):
        ...
        while True:
            try:
                udp_packet, _ = await asyncio.wait_for(self._rcv_queue.get(),
                                                       timeout=timeout)
