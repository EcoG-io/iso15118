...
class Config(SharedSettings):
    ...
    def load_envs(self, env_path: Optional[str] = None) -> None:
        ...
        # SDP request.
        self.enforce_tls = self.env.bool("SECC_ENFORCE_TLS", default=False)
        ...
        self.free_charging_service = self.env.bool(
            "FREE_CHARGING_SERVICE", default=False
        )

        # Indicates whether or not the installation of a contract certificate is free.
        # Should be configurable via OCPP messages.
        # Must be one of the bool values True or False
        self.free_cert_install_service = self.env.bool(
            "FREE_CERT_INSTALL_SERVICE", default=True
        )

        # Indicates if CPO integration is available to perform contract
        # certificate installation.
        self.use_cpo_backend = self.env.bool("USE_CPO_BACKEND", default=False)
        ...
        # Must be one of the bool values True or False
        self.allow_cert_install_service = self.env.bool(
        ...
        protocols = self.env.list("PROTOCOLS", default=self.default_protocols)
        ...
        auth_modes = self.env.list("AUTH_MODES", default=self.default_auth_modes)
        ...
        self.standby_allowed = self.env.bool("STANDBY_ALLOWED", default=False)
