...
def validate_bytes_value_range(
    ...
    if not min_val <= int_value <= max_val:
        raise ValueError(f"The value {int_value} is outside the allowed value "
...
def one_field_must_be_set(
        ...
        values: dict,
        mutually_exclusive: bool = False) -> bool:
    """
    ...
    if mutually_exclusive and len(set_fields) != 1:
        raise ValueError(f"Exactly one field must be set but {len(set_fields)} "
                         "are set instead. "
                         f"\nSet fields: {set_fields}"
                         f"\nField options: {field_options}")

    if len(set_fields) == 0:
        raise ValueError("At least one of these optional fields must be set "
