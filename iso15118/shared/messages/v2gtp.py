...
from iso15118.shared.messages.iso15118_20.common_types import (
    V2GMessage as V2GMessageV20, V2GRequest, V2GResponse)
...
class V2GTPMessage:
    ...
    def __init__(self,
                 protocol: Protocol,
                 payload_type: Union[DINPayloadTypes,
                                     ISOV2PayloadTypes,
                                     ISOV20PayloadTypes],
                 payload: bytes):
        """
        ...
        if protocol not in Protocol.options():
            raise InvalidProtocolError(f"'{protocol.name}' is not a "
                                       "valid protocol. Allowed: "
                                       f"{Protocol.allowed_protocols()}")

        if not self.is_payload_type_valid(protocol, payload_type):
            raise InvalidPayloadTypeError(f"Protocol {protocol} doesn't support"
    ...
    @classmethod
    def is_payload_type_valid(cls,
                              protocol: Protocol,
                              payload_type: int) -> bool:
        if (
                (protocol in [Protocol.ISO_15118_2, Protocol.UNKNOWN] and
                 payload_type not in ISOV2PayloadTypes.options()) or
                (protocol.ns.startswith('urn:iso:std:iso:15118:-20') and
                 payload_type not in ISOV20PayloadTypes.options())
        ):
            logger.error(f"{str(protocol)} does not support payload type "
                         f"{payload_type}")
            return False
    ...
    def is_header_valid(cls,
        ...
        is_valid: bool = True
        if len(header) != 8:
            logger.error(f"No proper V2GTP message, header is "
                         f"{len(header)} bytes long. Expected: 8 bytes")
            is_valid = False

        if protocol not in Protocol.options():
            logger.error(f"Unable to identify protocol version. "
                         f"Received: {protocol}")
            is_valid = False

        protocol_version = header[0]
        if protocol_version != V2GTPVersion.PROTOCOL_VERSION:
            logger.error(f"Incorrect protocol version '{protocol_version}' "
                         f"for V2GTP message. "
        ...
        if inv_protocol_version != V2GTPVersion.INV_PROTOCOL_VERSION:
            logger.error(f"Incorrect inverse protocol version "
                         f"'{inv_protocol_version}' for V2GTP message. "
                         f"Expected: {V2GTPVersion.INV_PROTOCOL_VERSION}")
            is_valid = False

        if not cls.is_payload_type_valid(protocol, cls.get_payload_type(
                header)):
            is_valid = False

        payload_length = cls.get_payload_length(header)
        if payload_length > INT_32_MAX:
            logger.error(f"Payload length of {payload_length} bytes for V2GTP "
                         f"message exceeds limit of {INT_32_MAX} bytes")
            is_valid = False

        if payload_length < 0:
            logger.error("Couldn't determine payload length of V2GTP message "
                         "(got -1)")
            is_valid = False
    ...
    def to_bytes(self) -> bytes:
        header = (self.protocol_version.to_bytes(1, 'big') +
        ...
        if len(data) >= 10:
            ...
            if cls.is_header_valid(protocol, header):
                if protocol.ns.startswith('urn:iso:std:iso:15118:-20'):
                    payload_type = ISOV20PayloadTypes(
                        cls.get_payload_type(header))
                else:
                    payload_type = ISOV2PayloadTypes(
                        cls.get_payload_type(header))
                return V2GTPMessage(protocol, payload_type, data[8:])
            raise InvalidV2GTPMessageError("Not a valid V2GTP message "
                                           "(header check failed)")
        raise InvalidV2GTPMessageError(f"Incoming data is too short to be "
                                       "a valid V2GTP message"
                                       f" (only {len(data)} bytes)")

    def __repr__(self):
        return (f"[Header = [{hex(self.protocol_version)}, "
