...
from iso15118.shared.messages.enums import Protocol, AuthEnum, Namespace
from iso15118.shared.messages.iso15118_2.body import (ServiceDiscoveryReq,
                                                      ServiceDiscoveryRes,
...
from iso15118.shared.messages.iso15118_2.datatypes import (
    SelectedService, SelectedServiceList, AuthOptions, EnergyTransferMode,
    ServiceCategory, EVSEProcessing, EVSENotification, ChargeProgress,
    ChargingSession, RootCertificateID, ServiceID, ACEVSEStatus)
...
from iso15118.shared.security import (load_cert, get_cert_cn, load_cert_chain,
                                      create_signature, load_priv_key,
                                      get_cert_issuer_serial, verify_signature,
                                      decrypt_priv_key, CertPath, KeyPath,
                                      KeyEncoding, to_ec_pub_key)
...
class SessionSetup(StateEVCC):
    ...
    def __init__(self, comm_session: EVCCCommunicationSession):
        ...
        super().__init__(comm_session,
                         TimeoutsShared.V2G_EVCC_COMMUNICATION_SETUP_TIMEOUT)
...
class ServiceDiscovery(StateEVCC):
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        offered_energy_modes = service_discovery_res.charge_service.\
            supported_energy_transfer_mode

        if EnergyTransferMode(value=self.comm_session.selected_energy_mode) not in \
                offered_energy_modes:
            self.stop_state_machine(f"Offered energy transfer modes "
        ...
        if len(self.comm_session.service_details_to_request) == 0:
            ...
            self.create_next_message(PaymentServiceSelection,
            ...
            self.create_next_message(ServiceDetail,
                                     service_detail_req,
    ...
    def select_energy_transfer_mode(self):
        ...
        if evcc_settings.RESUME_REQUESTED_ENERGY_MODE:
            logger.debug("Reusing energy transfer mode "
    ...
    def select_auth_mode(self, auth_option_list: List[AuthOptions]):
        ...
        if evcc_settings.RESUME_SELECTED_AUTH_OPTION:
            logger.debug("Reusing authorization option "
            ...
            if AuthOptions(value=AuthEnum.PNC_V2) in auth_option_list and \
               self.comm_session.is_tls:
    ...
    def select_services(self, service_discovery_res: ServiceDiscoveryRes):
        ...
        self.comm_session.selected_services.append(
            SelectedService(service_id=
                            service_discovery_res.charge_service.service_id))
        ...
        for service in service_discovery_res.service_list:
            ...
            if service.service_details.service_category == \
               ServiceCategory.CERTIFICATE and \
               self.comm_session.selected_auth_option and \
               self.comm_session.selected_auth_option == AuthEnum.PNC_V2 and \
               self.comm_session.ev_controller.is_cert_install_needed():
...
class PaymentServiceSelection(StateEVCC):
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        if self.comm_session.selected_auth_option and \
           self.comm_session.selected_auth_option == AuthEnum.PNC_V2:
            if self.comm_session.ev_controller.is_cert_install_needed():
                ...
                try:
                    signature = create_signature(
                        [(cert_install_req.id, to_exi(cert_install_req,
                                                      Namespace.ISO_V2_MSG_DEF))],
                        load_priv_key(KeyPath.OEM_LEAF_PEM, KeyEncoding.PEM))

                    self.create_next_message(CertificateInstallation,
                                             cert_install_req,
                                             Timeouts.CERTIFICATE_INSTALLATION_REQ,
                                             Namespace.ISO_V2_MSG_DEF,
                                             signature=signature)
                except PrivateKeyReadError as exc:
                    self.stop_state_machine("Can't read private key necessary to sign "
                                            f"CertificateInstallationReq: {exc.error}")
                    return
            else:
                try:
                    payment_details_req = PaymentDetailsReq(
                        emaid=EMAID(value=get_cert_cn(
                            load_cert(CertPath.CONTRACT_LEAF_DER))),
...
class CertificateInstallation(StateEVCC):
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        if not verify_signature(
                ...
                elements_to_sign=[(cert_install_res.contract_cert_chain.id,
                                  to_exi(cert_install_res.contract_cert_chain,
                                         Namespace.ISO_V2_MSG_DEF)),
                                  (cert_install_res.encrypted_private_key.id,
                                  to_exi(cert_install_res.encrypted_private_key,
                                         Namespace.ISO_V2_MSG_DEF)),
                                  (cert_install_res.dh_public_key.id,
                                  to_exi(cert_install_res.dh_public_key,
                                         Namespace.ISO_V2_MSG_DEF)),
                                  (cert_install_res.emaid.id,
                                  to_exi(cert_install_res.emaid,
                                         Namespace.ISO_V2_MSG_DEF))],
                leaf_cert=cert_install_res.cps_cert_chain.certificate,
                sub_ca_certs=[cert.certificate for cert in
                              cert_install_res.cps_cert_chain.sub_certificates],
                root_ca_cert_path=CertPath.V2G_ROOT_DER):
            self.stop_state_machine("Signature verification of "
                                    "CertificateInstallationRes failed")
            return

        try:
            decrypted_priv_key = decrypt_priv_key(
                encrypted_priv_key_with_iv=cert_install_res.encrypted_private_key.value,
                ecdh_priv_key=load_priv_key(KeyPath.OEM_LEAF_PEM,
                                            KeyEncoding.PEM),
            ...
            self.comm_session.ev_controller.store_contract_cert_and_priv_key(
                cert_install_res.contract_cert_chain.certificate,
                decrypted_priv_key)
        except DecryptionError:
            self.stop_state_machine("Can't decrypt encrypted private key for contract "
                                    "certificate")
            return
        except PrivateKeyReadError as exc:
            self.stop_state_machine("Can't read private key needed to decrypt "
...
class PaymentDetails(StateEVCC):
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        authorization_req = AuthorizationReq(
            id='id1',
            gen_challenge=payment_details_res.gen_challenge)

        try:
            signature = create_signature(
                [(authorization_req.id, to_exi(authorization_req,
                                               Namespace.ISO_V2_MSG_DEF))],
            ...
            self.stop_state_machine(f"Can't read private key to sign AuthorizationReq: "
                                    f"{exc.error}")
            return
...
class Authorization(StateEVCC):
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        if authorization_res.evse_processing == EVSEProcessing.FINISHED:
            ...
            self.create_next_message(ChargeParameterDiscovery,
            ...
            if self.comm_session.ongoing_timer >= 0:
                ...
                if elapsed_time > TimeoutsShared.V2G_EVCC_ONGOING_TIMEOUT:
                    self.stop_state_machine("Ongoing timer timed out for "
                                            "AuthorizationRes")
                    return
            else:
            ...
            self.create_next_message(Authorization,
                                     AuthorizationReq(),
                                     min(Timeouts.AUTHORIZATION_REQ,
                                         TimeoutsShared.V2G_EVCC_ONGOING_TIMEOUT
                                         - elapsed_time),
                                     Namespace.ISO_V2_MSG_DEF)
...
class ChargeParameterDiscovery(StateEVCC):
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        if charge_params_res.evse_processing == EVSEProcessing.FINISHED:
            ...
            charge_progress, schedule_id, charging_profile = \
            ...
            self.create_next_message(PowerDelivery,
                                     power_delivery_req,
            ...
            logger.debug("SECC is still processing the proposed charging "
            ...
            self.create_next_message(ChargeParameterDiscovery,
                                     charge_parameter_discovery_req,
                                     min(Timeouts.CHARGE_PARAMETER_DISCOVERY_REQ,
                                         TimeoutsShared.V2G_EVCC_ONGOING_TIMEOUT
                                         - elapsed_time),
                                     Namespace.ISO_V2_MSG_DEF)
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        if self.comm_session.charging_session_stop:
            ...
            self.create_next_message(ChargeParameterDiscovery,
                                     ...
                                     Timeouts.CHARGE_PARAMETER_DISCOVERY_REQ,
                                     Namespace.ISO_V2_MSG_DEF)
        elif self.comm_session.selected_energy_mode and \
                self.comm_session.selected_energy_mode.value.startswith("AC"):
            self.create_next_message(ChargingStatus,
                                     ChargingStatusReq(),
                                     Timeouts.CHARGING_STATUS_REQ,
                                     Namespace.ISO_V2_MSG_DEF)
        else:
            self.create_next_message(CurrentDemand,
...
class ChargingStatus(StateEVCC):
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        if charging_status_res.receipt_required and self.comm_session.is_tls:
            ...
            try:
                signature = create_signature(
                    [(metering_receipt_req.id, to_exi(metering_receipt_req,
                                                      Namespace.ISO_V2_MSG_DEF))],
                    load_priv_key(KeyPath.CONTRACT_LEAF_PEM, KeyEncoding.PEM))

                self.create_next_message(MeteringReceipt,
