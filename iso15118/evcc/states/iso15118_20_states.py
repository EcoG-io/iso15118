...
from iso15118.shared.messages.iso15118_20.common_messages import \
    SessionSetupRes, AuthorizationSetupReq, AuthorizationSetupRes, \
    CertificateInstallationReq, AuthorizationReq, \
    EIMAuthReqParams, PnCAuthReqParams
from iso15118.shared.messages.iso15118_20.common_types import MessageHeader, \
    RootCertificateID, V2GMessage as V2GMessageV20
from iso15118.shared.messages.xmldsig import X509IssuerSerial
from iso15118.shared.security import load_cert_chain, CertPath, \
    create_signature, load_priv_key, KeyPath, KeyEncoding, \
...
class SessionSetup(StateEVCC):
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        self.create_next_message(AuthorizationSetup,
                                 auth_setup_req,
...
class AuthorizationSetup(StateEVCC):
    ...
    def process_message(self, message: Union[SupportedAppProtocolReq,
        ...
        if auth_setup_res.cert_install_service and \
            ...
            try:
                signature = create_signature(
                    [(oem_prov_cert_chain.id, to_exi(oem_prov_cert_chain,
                                                     Namespace.ISO_V20_COMMON_MSG))],
                ...
                cert_install_req = CertificateInstallationReq(
                    ...
                    list_of_root_cert_ids=[RootCertificateID(
            ...
            if AuthEnum.PNC in auth_setup_res.auth_services:
                ...
                try:
                    signature = create_signature(
                        [(pnc_params.id, to_exi(pnc_params,
                                                Namespace.ISO_V20_COMMON_MSG))],
                        load_priv_key(KeyPath.OEM_LEAF_PEM, KeyEncoding.PEM))
                except PrivateKeyReadError as exc:
                    self.stop_state_machine("Can't read private key necessary to sign "
